...
export default function CourseTopics() {
  ...
  const handleSaveTopic = () => {
    ...
    const result = editingTopic ? updateTopic(editingTopic.id, topicForm) : createTopic(topicForm);
    if (result?.error) {
  ...
  if (courseId === 'custom') {
    ...
    title = course.title;
    topics = courseId === 'toeic-basic' ? TOEIC_BASIC_LESSONS_1_TO_50 : course.topics;
  }
  ...
  return (
    <main className="dash-main cv-subview" id="cv-topics-view" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="cv-topics-grid">
        ...
        {topics.map((topic) => {
          const total = Array.isArray(topic.words) ? topic.words.length : topic.wordCount || 0;
          const done = Array.isArray(topic.words) ? topic.words.filter((word) => remembered[word.id]).length : 0;
